...
export class RouteDetailsComponent {
  ...
  offerRide(form: NgForm) {
    ...
    this.RideService.offerRide(ride).subscribe({
      next: (res: any) => {
        ...
        this.HomeService.socket.on('requestedRides', (request: any) => {
          this.HomeService.requests = [...this.HomeService.requests, request];
          // this.HomeService.searchingRide = true;
          // this.removeListnerEvent.emit();
          // this.hide.emit();
        });
  ...
  takeRide() {
    ...
    this.RideService.takeRide(ride).subscribe({
      next: (res: any) => {
        this.HomeService.ongoingRide = res;

        this.HomeService.socket = io('http://localhost:3000');
        this.HomeService.socket.emit('ridetake', res);
        ...
        this.RideService.findRide(this.HomeService.route).subscribe({
          next: (matchedRides) => {
            ...
            this.HomeService.socket.on('rideIsFull', (ride: any) => {
              const index = this.HomeService.matchedRides.indexOf(ride);
              this.HomeService.matchedRides.splice(index, 1);
            });
          },
